...
_.loadExtension = (rootDir) => {
  ...
  const loadedExtensions = [] // TODO: This is kinda kludgy...
  ipcMain.handle(ipcChannel, _.loadedExtensionsResponder(loadedExtensions))

  const routers = []
  ipcMain.handle(ipcChannel, _.queryTabsResponder(routers))

  ipcMain.handle(ipcChannel, _.createTabResponder())

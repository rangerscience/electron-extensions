...
_.newBackgroundWindow = function newBackgroundWindow(extensionId) {
  const win = new BrowserWindow({
    width: 800,
    height: 600,
    show: !!process.env.IS_DEV,
    webPreferences: {
      preload: Path.join(__dirname, "./background_preload.js"),
      contextIsolation: !process.env.IS_DEV,
      nodeIntegration: true,
      devTools: true,
      nativeWindowOpen: true,
      additionalArguments: `extensionId=${extensionId}`
    }
  })
...
_.startBackgroundPage = function startBackgroundPage(
  ...
  const window = _.newBackgroundWindow(manifest.extensionId)

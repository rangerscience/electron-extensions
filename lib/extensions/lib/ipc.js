...
_.responder = (ipcChannel, backgroundWindow, contentWindows, manifest) => {
  return (event, message) => {
    if(message.envelope.extensionId != manifest.extensionId) { return }
    ...
    if(event.sender == backgroundWindow.webContents) {
      contentWindows.map((contentWindow) => {
        contentWindow.webContents.send(ipcChannel, message)
      })
    } else {
      backgroundWindow.webContents.send(ipcChannel, message)
    }
  }
}

function write_history(e) {
    return !is_user_debug_enabled() || ("undefined" != typeof LPhistory && null !== LPhistory || (LPhistory = new LPhistory_lib), LPhistory.write(e))
}
...
function reset_history() {
    return LPhistory = LPhistory || new LPhistory_lib, LPhistory.reset()
}

function dump_history() {
    return LPhistory = LPhistory || new LPhistory_lib, LPhistory.dump(null, g_isadmin ? LPhistory.LOG_ADMIN : LPhistory.LOG_USER)
}
...
function LPhistory_lib() {
    ...
    function a(e) {
        return !is_user_debug_enabled() || (H = H || [], !!e && (void 0 === e.timestamp && (e.timestamp = (new Date).getTime()), H.length > s && H.shift(), H.push(e), !0))
    }
    ...
    this.LOG_ADMIN = 2, this.LOG_DEBUG = 1, this.LOG_USER = 0, this.write = function(e) {
        if (!is_user_debug_enabled()) return !0;
        if (!e) return !1;
        var s;
        void 0 === e.timestamp && (e.timestamp = (new Date).getTime()), void 0 === e.loglevel && (e.loglevel = r(e));
        var i = e.tabid,
            t = e.docnum;
        if (void 0 === e.docnum && null !== g_CS_tops && i && (t = g_CS_tops[i]), null != t && i) {
            var o = "";
            if ("all" == t) o = ftd_get_topurl(i);
            else if ((s = getcsinfo(i, t)) && s.url) o = s.url;
            else if (g_CS[i] && g_CS[i][t]) {
                var n = g_CS[i][t];
                o = (o = gettaburl(n)) || n.name
            } else o = "";
            e.url = o
        }
        return a(e)
        ...
        var i = "",
            t;
        if (!is_user_debug_enabled()) return !e || i;
        if (void 0 === s && (s = Z.LOG_USER), !H) return console.error("empty history !"), !!e && i;
        for (t = 0; t < H.length; t++) {
            var o = H[t];
            if (o) {
                var n = o.loglevel;
                if (void 0 === o.loglevel && (n = Z.LOG_USER), s < n) continue;
                var r = "",
                    a = o.timestamp;
                r = void 0 === a || null === r ? "none?" : void 0 !== new Date(a).toUTCString ? new Date(a).toUTCString() : parseInt(a / 1e3).toString();
                var l = J({
                        prop: o.aid,
                        dflt: ""
                    }),
                    ...
                    g = "";
                p.match(/^\w{2}\d+$/) && Y[p] && (g = sprintf(Y[p], o.param1, o.param2, o.param3, o.param4, o.param5, o.param6, o.param7, o.param8, o.param9)) && (p = g);
                var b = J({
                        prop: o.submit,
                        dflt: "none"
                    }),
                ...
                switch (f) {
                    ...
                    case "autofillaid":
                    case "autologinaid":
                        z = sprintf("[submit=%s][doconfirm=%s][allowforce=%s]", b, h, _);
                        break;
                    case "fillfieldCS":
                        z = sprintf("[name=%s][manualfill=%s][is_launch=%s][originator=%s][delay=%dms][custom_js=%s]", D, v, y, A, k, E);
                        break;
                    case "fillbestCS":
                        z = sprintf("[username=%s][force_fillbest=%s][custom_js=%s]", S, w, E);
                        break;
                    case "submitCS":
                        z = sprintf("[submit_id=%s][delay=%dms][submit_js=%s]", L, k, C);
                        break;
                    case "check_autologin1":
                    case "check_autologin2":
                    case "check_autologin3":
                        z = sprintf("[dosubmit=%s]", b);
                        break;
                    case "fillfieldconfirm":
                        z = sprintf("[result=%s][manualfill=%s][allowforce=%s][fieldname=%s][force_fillbest=%s]", I, v, _, D, w);
                        break;
                    case "get_accts_success":
                    case "get_accts_fail":
                        z = sprintf("[size=%d][ready_state=%d][status=%d][lver=%d][sver=%d]", F, N, P, T, U);
                        break;
                    case "get_accts_local":
                        z = sprintf("[size=%d][status=%s][lver=%d][sver=%d]", F, O, T, U);
                        break;
                    case "showchangenotification":
                        z = sprintf("[spin=%s]", V);
                        break;
                    case "showaddnotification":
                        z = sprintf("[spin=%s][username=%s]", V, S);
                        break;
                    case "LoadData":
                        z = sprintf("[file=%s][num=%d]", q, R);
                        break;
                    case "errtrap":
                    case "errtrapCS":
                        z = sprintf("[errtype=%s][errmsg=%s][line=%d][stack=%s]", W, j, X, x);
                        break;
                    case "info":
                    case "decision":
                    default:
                ...
                var B = sprintf("[%s][%s]%s%s%s%s%s%s%s", f, r, d ? sprintf("[tabid=%s]", d) : "", u || 0 === u ? sprintf("[docnum=%s]", u) : "", c ? sprintf("[url=%s]", c) : "", l || 0 === l ? sprintf("[aid=%s]", l) : "", p ? sprintf("[msg=%s]", LPPerl.chomp(p)) : "", m ? sprintf("[docstate=%s]", m) : "", z);
                e ? i = i + B + "\n" : console.log(B)
            }
        }
        return !e || i

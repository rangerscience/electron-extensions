with((function() {
    ...
    function addWebsiteAbilities(T) {
        var i = document.documentElement.getAttribute("lastpass-extension");
        i = (i = i ? i.split(" ") : []).concat(T), document.documentElement.setAttribute("lastpass-extension", i.join(" "))
    }
    Topics = function() {
            var c = {};
            return {
                publish: function(T, i) {
                    Topics.get(T).publish(i)
                },
                get: function(T) {
                    var i = T && c[T];
                    if (!i) {
                        var s = [],
                            ...
                            o = function(T) {
                                var i = E(T); - 1 < i && s.splice(i, 1)
                            };
                        i = {
                            publish: function() {
                                for (var T = !0, i = s.slice(), E = 0, o = i.length; E < o && !1 !== T; ++E) try {
                                    "function" == typeof i[E] && (T = i[E].apply(window, arguments))
                                } catch (T) {
        ...
        function(s) {
            var i;
            s.requestFrameworkInitializer = (i = function(T, i) {
                var E = T("", {
                    name: "requestPort"
                });
                return E.onMessage.addListener(i),
                    function(T) {
                        E.postMessage(T)
                    }
            }, function(T) {
                return i(chrome.runtime && chrome.runtime.connect || parent.chrome.runtime.connect, T)
            }), s.installBinary = function(i, E) {
                var o = getBG();
                chrome.permissions.contains({
                    permissions: ["nativeMessaging"]
                }, function(T) {
                    i || T ? o.openURL(o.base_url + "installer/") : s.requestNativeMessaging(E)
                })
            }, s.requestNativeMessaging = function(T) {
                var i = getBG();
                i.Preferences.set("native_messaging_asked", "1"), T ? window.open("/native_messaging.html?hidenever=1") : void 0 !== chrome.permissions && chrome.permissions.request({
                    permissions: ["nativeMessaging", "privacy"]
                }, function(T) {
                    T && alert(i.gs("Please restart your browser to finish enabling native messaging."))
                })
            }
        }(LPPlatform), addWebsiteAbilities("fedlogin"),
        function() {
            var o = null,
                T = function(T) {
                    T.fromExtension = !0, o && o.target.postMessage(T, o.origin)
                },
                s = LPPlatform.requestFrameworkInitializer(T),
                i = function(T) {
                    var i = T.data,
                        E = i && i.data;
                    i.type && (null === o && (T.origin === window.origin || "undefined" != typeof bg && bg.get("base_url") === T.origin + "/") && (o = {
                        target: T.source,
                        origin: T.origin
                    }), o && o.target === T.source && o.origin === T.origin && (i.fromExtension || E && E.cmd && "FederatedLogin" !== E.cmd[0] && "getVersion" !== E.cmd || s(i)))
                };

with((function() {
    var getProxy = function(globalContext, proxifyGlobalContext) {
    ...
    return getProxy(this, false);
})()) {
    var processAcctsIframeMessage = function(t) {
            "getdata" === t.data.msg ? bg.processCS(null, {
                cmd: "ipcgetdata",
                url: t.data.url,
                callback: function(e) {
                    "ipcgotdata" === e.cmd && t.source.postMessage(e, t.origin)
                }
            }, null) : "closeiframe" === t.data.msg ? bg.closeSettingsIframe() : "refreshsites" === t.data.msg ? bg.refreshsites() : "storeaccountlinktoken" === t.data.msg ? "function" == typeof bg.storeAccountLinkToken && bg.storeAccountLinkToken(t.data.payload) : "websiteevent" == t.data.msg && (g_websiteeventtarget = t, document.getElementById("eventtype").value = t.data.eventtype, document.getElementById("eventdata1").value = t.data.eventdata1, document.getElementById("eventdata2").value = t.data.eventdata2, document.getElementById("eventdata3").value = t.data.eventdata3, document.getElementById("eventdata4").value = t.data.eventdata4, document.getElementById("eventdata5").value = t.data.eventdata5, website_event())
        },
        g_websiteeventtarget = LPVARS.g_ipctarget = null;

    function forward_website_event_response(e) {
        for (var t = 0; t < parent.frames.length; t++)
            if (parent.frames[t].document.getElementById("lpwebsiteeventform") && "function" == typeof parent.frames[t].LPVARS.website_event_callback) {
                parent.frames[t].LPVARS.website_event_callback(e);
                break
            }
    }

    function addWebsiteAbilities(e) {
        var t = document.documentElement.getAttribute("lastpass-extension");
        t = (t = t ? t.split(" ") : []).concat(e), document.documentElement.setAttribute("lastpass-extension", t.join(" "))
    }
    this.website_event = function() {
        var e = document.getElementById("eventtype").value;
        "function" == typeof lpdbg && lpdbg("vault", "new vault got website event: " + e);
        var t = "undefined" != typeof bg ? bg : getBG();
        LPVARS.base_url = t.get ? t.get("base_url") : t.base_url;
        var a = {
            cmd: e,
            url: LPVARS.base_url,
            callback: LPVARS.website_event_callback
        };
        switch (e) {
            case "refresh":
                a.from = document.getElementById("eventdata1").value, a.type = document.getElementById("eventdata2").value;
                break;
            case "logout":
            case "logoff":
            case "clearcache":
                break;
            case "keyweb2plug":
                a.cmd = "web2plug", a.key = document.getElementById("eventdata1").value, a.username = document.getElementById("eventdata2").value, a.rsa = document.getElementById("eventdata3").value;
                break;
            case "checkmultifactorsupport":
                a.type = document.getElementById("eventdata1").value;
                break;
            case "setupsinglefactor":
                a.type = document.getElementById("eventdata1").value, a.username = document.getElementById("eventdata2").value, a.password = document.getElementById("eventdata3").value, a.silent = document.getElementById("eventdata5").value, "1" != document.getElementById("eventdata5").value && g_websiteeventtarget && g_websiteeventtarget.source.postMessage({
                    cmd: e,
                    result: "working"
                }, g_websiteeventtarget.origin);
                break;
            case "rsadecrypt":
                a.sharerpublickeyhex = document.getElementById("eventdata1").value, a.sharekeyenchexsig = document.getElementById("eventdata2").value, a.sharekeyenchex = document.getElementById("eventdata3").value, a.sharekeyhex = document.getElementById("eventdata4").value;
                break;
        ...
        a.callback = this.website_event_callback, t.processCS(null, a, null)
    }, this.website_event_callback = function(e) {
        if ("function" == typeof lpdbg && lpdbg("vault", "new vault got website event callback: " + e.cmd), "checkmultifactorsupport" == e.cmd) {
            if (!document.getElementById("lpwebsiteeventform")) return void forward_website_event_response(e);
            document.getElementById("eventdata4").value = e.type, document.getElementById("eventdata3").value = e.result, g_websiteeventtarget && g_websiteeventtarget.source.postMessage(e, g_websiteeventtarget.origin)
        } else if ("setupsinglefactor" == e.cmd) {
            if (!document.getElementById("lpwebsiteeventform")) return void forward_website_event_response(e);
            document.getElementById("eventdata4").value = e.result, g_websiteeventtarget && g_websiteeventtarget.source.postMessage(e, g_websiteeventtarget.origin)
        } else "ipcgotdata" == e.cmd && LPVARS.g_ipctarget.source.postMessage(e, LPVARS.g_ipctarget.origin)
    }, addWebsiteAbilities("acctsiframe"), window.addEventListener("message", function(e) {
        e.origin === location.origin && processAcctsIframeMessage(e)
    });
}

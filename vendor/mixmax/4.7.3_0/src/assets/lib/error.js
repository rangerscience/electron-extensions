 ...
 * and error.captureException should be called.
 */
...
/**
 * Wrapper for Raven's captureException.
 */
var captureException = function(error, ravenOptions) {
    // Log to console to help debugging.
    console.error(error.stack || error, ravenOptions);

    Raven.captureException.apply(Raven, arguments);
};


var error = errorLegacy;
error.captureMessage = captureMessage;
error.captureException = captureException;
